...
- Quality-first pipeline: OpenAI turns a small set of current-event cues into a museum-grade fine-art wallpaper prompt and renders it. Headlines inform the image but should never appear as visible text.
...
- Configure env: copy `.env.example` to `.env` and set values.
  ...
  - `OPENAI_QA_MODEL`: optional; unused by the default imagery-only path.
...
- Edit `image.config.json` to tune style, vibe, negative prompts, feeds, and keyword limits.
  ...
  - `headlinePromptLimit`: number of current-event source cues sent to the prompt refiner; lower values reduce text-model spend
  - `stylePool`: random classical art and artful photography styles chosen per run
...
- `metadata` includes the source cues, extracted keywords, keyword base prompt, randomized style, optional QA result, retry status, and final image selection.
...
- Refine: OpenAI (`openaiTextModel`, default `gpt-5.4-mini`) crafts one polished imagery prompt from compact current-event source cues, extracted keywords, time-of-day, vibe, and a randomly selected fine-art style from `stylePool`.
  - The refiner is instructed to translate current events into lived human consequence, atmosphere, composition, and emotional stakes.
  - Photographic styles are treated as fine-art photography, not literal press or stock imagery.
  - The generated image prompt is instructed not to quote, paraphrase, display, or describe headline text.
...
- QA: The default imagery-only path skips embedded-text QA.
